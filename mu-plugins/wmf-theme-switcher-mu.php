<?php
/*
    Plugin Name: WMF Theme Switcher MU Addon
    Plugin URI: http://themeforest.net/user/Webbu
    Description: This is addon module for WMF Theme Switcher Plugin to disable unused plugins while switching the themes.
    Version: 1.0.0
    Author: Webbu
...
if (!is_admin() && ( ! defined( 'DOING_AJAX' ) || ! DOING_AJAX )) {
    if (!class_exists('WMF_Mobile_Detect')) {
    ...
    if (!class_exists('Wmf_Theme_Switcher_MU')) {

        class Wmf_Theme_Switcher_MU
        {

            public function __construct(){
                add_filter( 'option_active_plugins', [$this, 'wmfthsw_disable_excluded_plugins'] );
                add_action( 'plugins_loaded',[$this, 'wmf_remove_filterafteruse'],9999 );
            }

            public function wmf_remove_filterafteruse(){
                remove_filter( 'option_active_plugins',[$this, 'wmfthsw_disable_excluded_plugins'],0);
            }

            public function wmfthsw_disable_excluded_plugins($plugins){
                    ...
                    if (!empty($options)) {
                        ...
                        if ($filters_apply) {
                            ...
                            foreach ($plugin_options as $plugin_option) {
                                if ($plugin_option != 'wmf-theme-switcher/wmf-theme-switcher.php') {
                                    $key = array_search( $plugin_option , $plugins );
                                    if ( false !== $key ) {
                                        unset( $plugins[$key] );
                                    }
                                }
